...
exports.default = (err, __, res, next) => {
    ...
    if (err instanceof api_1.APIError) {
        ...
        if (Array.isArray(err)) {
            apiError = new api_1.APIError(reportError2httpStatusCode(err[0]), err);
        }
        else if (err instanceof report.factory.errors.Report) {
            apiError = new api_1.APIError(reportError2httpStatusCode(err), [err]);
        }
        else {
            // 500
            apiError = new api_1.APIError(http_status_1.INTERNAL_SERVER_ERROR, [new report.factory.errors.Report('InternalServerError', err.message)]);
        }
    }
 ...
 * @param err Reportエラー
 */
function reportError2httpStatusCode(err) {
